...
import PodBoard from '@/components/pods/PodBoard';
import {
  Bar,
  BarChart,
  CartesianGrid,
...
export default function PodsPage({ reportsOnly = false }: { reportsOnly?: boolean }) {
  ...
  useEffect(() => {
    const next = searchParams.get('branch') ?? '';
    setBranchFilter((current) => (current === next ? current : next));
  }, [searchParams]);

  const applyBranchFilter = (value: string) => {
    setPage(0);
  ...
  const query = useMemo(
    ...
    [page, pageSize, search, statusFilter, branchFilter, sortBy, sortDir],
  );

  useEffect(() => {
    setLoading(true);
    Promise.all([
      podsApi.summary(branchFilter ? { branch: branchFilter } : undefined),
      podsApi.status({
      ...
      podsApi.list(query),
      podsApi.list({ page: 1, pageSize: 1000, branch: branchFilter || undefined }),
    ])
    ...
    // Seed historyPodId from the first result once; do not refetch the list when it changes.
    ...
    <Box>
      ...
      {view === 'board' && !reportsOnly ? (
        <PodBoard
          ...
          onBranchFilter={applyBranchFilter}
          onAdd={openCreate}
          onEdit={openEdit}
          onExport={() => void download('xlsx')}
        ...
        {reportsOnly ? (
          ...
          >
            <ToggleButton value="board">
              <ViewKanbanIcon fontSize="small" sx={{ mr: 0.75 }} />
              Board
            </ToggleButton>
            <ToggleButton value="reports">
              <AssessmentIcon fontSize="small" sx={{ mr: 0.75 }} />
              Reports
            </ToggleButton>
          </ToggleButtonGroup>
        )}
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={String(row.id)} hover>
                    <TableCell>
                      <Link component={NextLink} href={`/pods/${row.id}`}>
                        {String(row.name)}
                      </Link>
                    </TableCell>
                    <TableCell>{formatPodBranch(row.branch as string | null)}</TableCell>
