...
import { isAggregateMemberName, normalizeKey } from '@/lib/shared';
...
export class BdgService {
  ...
  async findAll(query: BdgListQuery) {
    ...
    const [raw, total, allForTotals] = await Promise.all([
      ...
      prisma.bdgMember.count({ where }),
      prisma.bdgMember.findMany({ where }),
    ]);
  ...
  async findOne(id: string) {
    const member = await prisma.bdgMember.findUnique({ where: { id } });
    if (!member) throw new ApiError(404, 'BDG member not found');
    if (isAggregateMemberName(member.memberName)) {
      throw new ApiError(404, 'BDG member not found');
    }
  ...
  async create(dto: BdgUpsertDto) {
    ...
    const existing = await prisma.bdgMember.findUnique({
    ...
    const created = await prisma.bdgMember.create({
  ...
  async update(id: string, dto: BdgUpsertDto) {
    const existing = await prisma.bdgMember.findUnique({ where: { id } });
    if (!existing) throw new ApiError(404, 'BDG member not found');
    this.assertValidMember(dto.memberName ?? existing.memberName);
    this.assertNonNegative(dto);
    const normalized = normalizeKey(dto.memberName);
    if (normalized !== existing.normalizedMemberName) {
      const clash = await prisma.bdgMember.findUnique({
        where: { normalizedMemberName: normalized },
      });
    ...
    const updated = await prisma.bdgMember.update({
      where: { id },
      data: {
  ...
  async remove(id: string) {
    const existing = await prisma.bdgMember.findUnique({ where: { id } });
    if (!existing) throw new ApiError(404, 'BDG member not found');
    await prisma.bdgMember.delete({ where: { id } });
    return { id, deleted: true };
  }

  async purgeAggregateRows() {
    const all = await prisma.bdgMember.findMany();
    const bad = all.filter((m) => isAggregateMemberName(m.memberName));
    if (bad.length) {
      await prisma.bdgMember.deleteMany({
  ...
  async summary() {
    ...
    const members = await prisma.bdgMember.findMany();
  ...
  private assertValidMember(name: string) {
    ...
    if (!trimmed) {
      throw new ApiError(400, 'BDG Member is required');
    }
  ...
  private assertNonNegative(dto: BdgUpsertDto) {
    ...
    for (const f of fields) {
      ...
      if (typeof v === 'number' && v < 0) {
        throw new ApiError(400, `Lead values cannot be negative (${f})`);
      }
    }
  }
...
export const bdgService = new BdgService();
